...
function switchLanguage(lang, btnElement = null) {
    ...
    // テキスト置換（HTMLタグを維持するためinnerHTMLを使用）
...
function initDb() {
    ...
    rawData.forEach(c => {
        ...
        card.innerHTML = `
            ...
            <img src="${charImages[c.en] || ''}" class="char-img">
        `;
...
/**
 * ★スマホ動画再生対策★
 * 画面が最初にタッチされた瞬間に、停止しているすべての動画を強制的に再生させます。
 */
document.addEventListener('touchstart', function() {
    ...
    vids.forEach(v => {
        if (v.paused) {
            v.play().catch(err => {
                console.log("スマホでの動画再生がブロックされました:", err);
            });
        }
    });
}, { once: true }); // 最初の1回だけ実行
