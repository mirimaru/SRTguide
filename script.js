...
function initDb() {
    ...
    rawData.forEach(c => {
        ...
        card.innerHTML = `
            ...
            <img src="${charImages[c.en] || ''}" style="position: absolute !important; bottom: -10px !important; right: -10px !important; height: 240px !important; width: auto !important; opacity: 0.4 !important; z-index: 1 !important;">
        `;
...
function initPBuff() {
    ...
    for (const [posName, chars] of Object.entries(pBuffData)) {
        ...
        chars.forEach(char => {
            const card = document.createElement('div'); 
            card.className = 'pbuff-card relative overflow-hidden';
            if(posColors[pCode]) card.classList.add(...posColors[pCode].split(' '));
            const cName = currentLang === 'ja' ? char.name : char.en;
            let bHtml = `<div class="char-content p-10 relative z-10 min-h-[220px]"><h3 class="text-3xl font-black italic text-orange-500 mb-6">${cName}</h3><div class="space-y-3">`;
            ...
            bHtml += `</div></div><img src="${charImages[char.en] || ''}" style="position: absolute !important; bottom: -5px !important; right: -10px !important; height: 200px !important; width: auto !important; opacity: 0.5 !important; z-index: 1 !important;">`;
